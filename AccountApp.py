...
class CreatAccountSystem(BaseManagementUI):
    ...
    def on_add(self) -> None:
        ...
        existing = [r[1] for r in self.db.select_all()]
        if data['帳號'] in existing:
            self.set_status('Add data already!', 'red')
            self.show_warning('資料已經寫入，請重新輸入!')
            return

        insert_data = (data['帳號'], data['密碼'])
...
if __name__ == '__main__':
    CreatAccountSystem(iconbitmap=r'..\Image\login.ico').run()