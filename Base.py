...
class Database:
    ...
    def _create_table(self) -> None:
        ...
        sql : str = f"""CREATE TABLE IF NOT EXISTS '{self.table_name}' (
    ...
    def insert(self, data: Tuple) -> bool:
        ...
        try:
            ...
            cols : str         = ', '.join(f"'{c}'" for c in self.columns[1:])
...
class BaseManagementUI:
    ...
    def create_form(self, fields: List[Tuple[str, bool]]) -> tk.Frame:
        ...
        tk.Label(frame, text = self.win.title(), font = font_title).grid(row = 0, column = 0, columnspan = 2, padx = 10, pady = 5)

        for i, (field_name, is_password) in enumerate(fields, start = 1):
            tk.Label(frame, text = field_name, font = font_field).grid(row = i, column = 0, padx = 5, pady = 2)

            entry = tk.Entry(frame, bg = "lightyellow", fg = "black", font = font_field, borderwidth = 3, show = '*' if is_password else '')
    ...
    def create_buttons(self, commands: Dict[str, Callable]) -> tk.Frame:
        ...
        for i, name in enumerate(button_names[:4]):
            if name in commands:
                btn = tk.Button(frame, text = name, font = font, command = commands[name])
                btn.grid(row = 0, column = i, padx = 5, pady = 5, sticky = 'nswe')

        if '開啟' in commands:
            btn = tk.Button(frame, text = '開啟', font = font, command = commands['開啟'])
            btn.grid(row = 1, column = 0, columnspan = 2, padx = 5, pady = 5, sticky = 'nswe')

        if '結束' in commands:
            btn = tk.Button(frame, text = '結束', font = font, command = commands['結束'])
            btn.grid(row = 1, column = 2, columnspan = 2, padx = 5, pady = 5, sticky = 'nswe')
